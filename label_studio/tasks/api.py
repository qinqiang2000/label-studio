...
class AnnotationFieldAnnotationsAPI(generics.RetrieveUpdateAPIView):
    """
    API for managing field annotations (comments/notes on form fields)
    """
    permission_required = ViewClassPermission(
        GET=all_permissions.annotations_view,
        PUT=all_permissions.annotations_change,
    ...
    def get_object(self):
        """Override to handle both integer and string IDs"""
        pk = self.kwargs.get('pk')
        try:
            # Try to get by primary key (integer)
            if pk.isdigit():
                return Annotation.objects.get(pk=int(pk))
            else:
                # Try to get by unique_id field for string IDs like "TkNUb"
                return Annotation.objects.get(unique_id=pk)
        except Annotation.DoesNotExist:
            # Try the other approach if first attempt failed
            try:
                if pk.isdigit():
                    return Annotation.objects.get(unique_id=pk)
                else:
                    # Last resort: try pk as string (though this is unlikely to work)
    ...
    def get(self, request, *args, **kwargs):
        """Get field annotations for an annotation"""
        annotation = self.get_object()
        
        # Extract field_annotations from result.meta if it exists
        field_annotations = {}
        if annotation.result:
            # If result is a list, look for meta in the first result item
            if isinstance(annotation.result, list) and annotation.result:
                for result_item in annotation.result:
                    if isinstance(result_item, dict) and 'meta' in result_item:
                        ...
                        break
            # If result is a dict with meta
            elif isinstance(annotation.result, dict) and 'meta' in annotation.result:
                field_annotations = annotation.result.get('meta', {}).get('field_annotations', {})
        
        return Response({'field_annotations': field_annotations})
    
    def put(self, request, *args, **kwargs):
        """Update field annotations for an annotation"""
        annotation = self.get_object()
        field_annotations = request.data.get('field_annotations', {})
        ...
        if not isinstance(annotation.result, list):
            annotation.result = []
        
        # Strategy: Always try to add field_annotations to an existing valid result item
        # Find the first valid result item (has type and from_name)
        target_result_index = -1
        
        for i, result_item in enumerate(annotation.result):
            if isinstance(result_item, dict) and result_item.get('type') and result_item.get('from_name'):
                target_result_index = i
                break
        
        if target_result_index != -1:
            # Add meta to existing valid result item
            if 'meta' not in annotation.result[target_result_index]:
                annotation.result[target_result_index]['meta'] = {}
            annotation.result[target_result_index]['meta']['field_annotations'] = field_annotations
        else:
            # If no valid result items exist, create a complete result item
            # This should rarely happen in practice
            annotation.result.append({
                'type': 'meta',
                'from_name': 'field_annotations',
                'to_name': 'field_annotations',
                'meta': {'field_annotations': field_annotations}
            })
        
        # Clean up any orphaned meta-only result items (fix existing data)
        annotation.result = [
            item for item in annotation.result 
            if not (isinstance(item, dict) and 
                   list(item.keys()) == ['meta'] and 
                   'field_annotations' in item.get('meta', {}))
        ]
