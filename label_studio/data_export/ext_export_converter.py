...
def extract_annotations_and_data(json_path: str) -> dict:
    ...
    输出为包含(id, filename, annotation_text, prediction_text)元组的列表。
    如果任何一个为空，则打印err并跳过该项。
    :param json_path: json文件路径
    :return: List[(id, filename, annotation_text, prediction_text)]
    """
    results = []
    with open(json_path, "r", encoding="utf-8") as f:
        ...
        for item in data:
            ...
            if not item_id or not filename:
                print(
                    f"err: id={item_id}, filename={filename}, annotation_text={annotation_text}, prediction_text={prediction_text}"
                )
                continue
            
            results.append((item_id, filename, annotation_text, prediction_text))
    return results
...
def export_to_excel(json_path: str, output_data: str):
    ...
    def process_text_data(text, worksheet, start_row, item_id, filename):
        """
        处理文本数据并写入工作表
        :param text: JSON文本
        ...
        try:
            ...
            if isinstance(data, list):
                # 如果是空列表，仍然插入一行，只包含id和filename
                if len(data) == 0:
                    worksheet.cell(row=current_row, column=1, value=item_id)  # id列
                    worksheet.cell(row=current_row, column=2, value=filename)  # filename列
                    current_row += 1
                else:
                    for item in data:
                        # 设置基本字段
                        ...
                        # 如果没有page字段，默认填入1
                        if "page" not in item:
                            worksheet.cell(row=current_row, column=3, value=1)  # page列
                        
                        # 处理其他字段
                        for col_idx, field in enumerate(fields[2:], 3):  # 从第3列开始（跳过id和filename）
                            if field in item:
                                value = item[field]
    ...
    for item_id, filename, annotation_text, prediction_text in results:
        # 处理 annotation_text
        annotation_row = process_text_data(annotation_text, ws_annotation, annotation_row, item_id, filename)
        
        # 处理 prediction_text
        prediction_row = process_text_data(prediction_text, ws_prediction, prediction_row, item_id, filename)
