...
export const EvaluationMatchingStrategyConfig = ({
  ...
  return (
    <Block name="matching-strategy-config">
      <Elem name="header">
        <Elem name="title">票据匹配策略</Elem>
        <Elem name="description">配置如何将标准票据与预测票据进行配对比较</Elem>
      </Elem>

      {/* 主要匹配模式选择 */}
      <Elem name="matching-mode">
        <label>
          <Checkbox checked={usePositionMatching} onChange={handleMatchingModeChange} disabled={disabled} />
          ...
          <Elem name="field-matching-section">
            ...
            <Elem name="primary-fields-config">
              ...
              <Elem name="current-fields">
                ...
                <Elem name="fields-list-draggable">
                  {primaryFields.length > 0 ? (
                    ...
                    <Elem name="no-fields">未配置（将使用位置匹配）</Elem>
                  )}
                </Elem>
              </Elem>
              ...
              <Elem name="add-field-section">
                <Button onClick={() => setShowFieldSelector(!showFieldSelector)} disabled={disabled} size="small">
              ...
              {primaryFields.length > 1 && (
                <Elem name="sorting-tips">
                  ...
                  <ul>
                    <li>
                      字段的顺序影响匹配结果，请将<strong>重要性高的字段放在前面</strong>
                    </li>
                    <li>
                      系统会对未匹配数据尝试降级匹配，如：[{primaryFields.join(", ")}] → [
                      {primaryFields.slice(1).join(", ")}] → [{primaryFields[primaryFields.length - 1]}]
                    </li>
                    <li>
                      <strong>拖拽字段</strong>可调整顺序，重要字段在前可确保优先匹配
                    </li>
                  </ul>
                </Elem>
              )}
      ...
      <Elem name="config-preview">
        ...
        <Elem name="preview-content">
          {usePositionMatching ? "顺序匹配：按位置一对一配对" : `字段匹配: [${primaryFields.join(", ")}]`}
        </Elem>
      </Elem>
    </Block>
  );
