...
export const createPermissionChecker = (userPermissions = []) => {
  ...
  return {
    ...
    // 检查菜单权限
    hasMenuPermission: (menuKey) => permissionSet.has(MENU_PERMISSIONS[menuKey]),

    // 检查操作权限
    hasActionPermission: (actionKey) => permissionSet.has(ACTION_PERMISSIONS[actionKey]),

    // 检查管理权限
    hasAdminPermission: (adminKey) => permissionSet.has(ADMIN_PERMISSIONS[adminKey]),

    // 检查页面操作权限
    hasPageOperationPermission: (operationKey) => permissionSet.has(PAGE_OPERATION_PERMISSIONS[operationKey]),

    // 检查表单字段权限
    hasFieldPermission: (fieldKey) => permissionSet.has(FIELD_PERMISSION_PERMISSIONS[fieldKey]),
    ...
    hasAnyPermissionInCategory: (category) => {
      ...
      return Object.values(categoryPermissions).some((permission) => permissionSet.has(permission));
    },
  };
};
...
export const validatePermissions = (permissions) => {
  ...
  return permissions.filter((permission) => allValidPermissions.includes(permission));
};
