...
const HtxResizableView = observer(({ item }) => {
  const leftMinWidth = Number.parseInt(item.leftminwidth) || 300;
  const rightMinWidth = Number.parseInt(item.rightminwidth) || 250;
  const rightInitialWidth = Number.parseInt(item.rightinitialwidth) || 350;
  ...
  // 防抖保存用户偏好设置
  const saveUserPreferenceDebounced = useCallback(
    (width) => {
      // 清除之前的延时保存
  ...
  // 内存优化：防抖设置宽度
  const setRightPanelWidthOptimized = useCallback(
    (width) => {
      if (Math.abs(width - rightPanelWidth) > 1) {
        // 只有变化超过1px才更新
    ...
    (e) => {
      ...
      const handleMouseMove = (moveEvent) => {
        ...
        animationIdRef.current = requestAnimationFrame(() => {
          ...
          // Apply constraints
          const clampedWidth = Math.min(Math.max(newRightWidth, rightMinWidth), containerWidth - leftMinWidth);
